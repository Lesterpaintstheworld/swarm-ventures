...
const InvestmentsTable = () => {
  ...
  if (error) {
    return (
      <div className="bg-red-900/30 border border-red-800 text-red-200 px-4 py-3 rounded-lg text-center">
        {error}
      </div>
    );
  }
  ...
  if (investments.length === 0) {
    return (
      <div className="text-center py-8 text-gray-400">
        No investments have been made yet. Be the first to invest!
      </div>
    );
  }
      ...
      <table className="min-w-full divide-y divide-gray-700">
        ...
        <tbody className="divide-y divide-gray-700">
          {investments.map((investment, index) => (
            <tr key={investment.id || index} className="hover:bg-gray-900/50">
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm">
                {investment.signature ? (
                  <a 
                    href={`https://solscan.io/tx/${investment.signature}`}
                    target="_blank"
  ...
  const handleInvest = async (e) => {
    ...
    try {
      ...
      try {
        ...
        // Create a connection to the Solana network with hardcoded Helius API key
  ...
  return (
    <div className="min-h-screen bg-black text-white relative overflow-hidden z-10" style={{ backgroundColor: '#000000' }}>
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        {/* Investments Table */}
        <section className="py-10 relative z-10">
          <div className="metallic-card p-8 md:p-10 rounded-xl max-w-5xl mx-auto">
            <h3 className="text-2xl font-bold mb-8 gold-text text-center">Recent Investments</h3>
            <InvestmentsTable />
          </div>
        </section>
